...
def set_val_entry_id():
    ...
    result = db.session.query(func.max(Recognized.id)).one()
    max_id = int(result[0])
...
def infer(args):
    ...
    for img in args.imgs:
        ...
        if confidence > 0.78:
            ...
            conf_lvl = float("{0:.2f}".format(confidence))
            datetime_now = datetime.strptime(img[23:-4], '%Y-%m-%d-%H-%M-%S')
            max_id = set_val_entry_id()
            query = "SELECT setval('recognized_id_seq', :new_id)"
            db.session.execute(query, {'new_id': max_id})
            db.session.commit()
            # entring new entry into the database
            new_entry = Recognized(entrance_id=entrance_id,
                                person_id=person_id,
            ...
            # file has been processed, move it to a different directory
            ### The following two lines are for demo only. This is to visually show pretend faces on Demo Day ###
            os.chdir('/home/vagrant/src/fr_project/thing/static/demoFaces')
            os.system("mv " + img[23:] + " /home/vagrant/src/fr_project/thing/static/demoPassed")

            #### The following 4 lines are for non-demo. It uses real still and animated images from the NestCam ####
            ...
            print "Cannot confidently recognize " + img[23:-4] + ". Please check manually."
            entrance_id = 3
            building_id = 2
            person_id = 7
            conf_lvl = float("{0:.2f}".format(confidence))
            ...
            ### The following two lines are for demo only. This is to visually show pretend faces on Demo Day ###
            os.chdir("/home/vagrant/src/fr_project/thing/static/demoFaces")
            os.system("mv " + img[23:] + " /home/vagrant/src/fr_project/thing/static/demoMnChk")
